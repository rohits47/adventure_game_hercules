...
public class Person
{
	...
	{	
		...
		if (room != null)
		{
			if ((this.getName()).equals("Medusa") || (this.getName()).equals("Cereberus") || (this.getName()).equals("Minotaur") || (this.getName()).equals("NemeanLion") || (this.getName()).equals("LerneanHydra")) // prevents task-related people from moving
			{
				// doesn’t do anything
				// this may be bad coding form, since I have an empty if statement, but I don't know how else to specify that if this is medusa, hydra, minotaur, or nemean lion
	...
	public void takeSomething()
	{
		...
		if (temp != null)
		{
			...
			else // instance of Person
			{
				...
				if (tempName.equals("Medusa") || (tempName.equals("Minotaur")) || (tempName.equals("NemeanLion")) || (tempName.equals("LerneanHydra")) || (tempName.equals("Cereberus"))) // prevents items related to the tasks from changing ownership
				{
					temp = null;
				}
				if (temp != null)
	...
	public void take(Thing thing)
	{
		...
		else
		{
			if ((thing.getOwner()) instanceof Room) // if thing is owned by a room
			{
				...
				{
					System.out.println("After taking the hide, it might be helpful to find a way to get the hide to Hades.");
					System.out.println("Hint 1: Hades lives in the Underworld.");
					System.out.println("Hint 2: Check your commands list to see if you can find a way of getting places.");
				}
			}
			else
			{
				...
				if (personOwner == this)
				{
					System.out.println(getName() + " already has that!"); // if person tries to take a thing that person already owns
				}
				else
				{
					...
					else if ((thing.getName()).equals("NemeanLionHide") && (personOwner.getName()).equals("NemeanLion"))
					{
						personOwner.say("RRRROOOOOOOOOAAAAAAAARRRRRRR!!!!!!!");
						System.out.println("The ground quakes from the strength of the lions roar.");
						System.out.println(personOwner.getName() + " violently attacks " + name + ".");
						System.out.println(name + " has died. Next time, " + name + " might not want to take the hide of a LIVING lion.");
					...
					{
						personOwner.say("SNORT. My horns have more strength than that, should you wish to rip them from my thick skull.");
						System.out.println(personOwner.getName() + " gores " + name + " on his golden horns.");
						System.out.println(name + " has died. Next time, " + name + " might not want to take the horns of a LIVING minotaur.");
						if (things.size() > 0) // death
						{
							for (int i = 0; i < things.size(); i++)
							{
					...
					else if ((thing.getName()).equals("MedusasHead") && (personOwner.getName()).equals("Medusa"))
					{
						...
						System.out.println(name + " has died. Next time, " + name + " might not want to take the head off of a LIVING medusa.");
						if (things.size() > 0) // death
						{
							for (int i = 0; i < things.size(); i++)
							{
					...
					else if ((thing.getName()).equals("TonicOfImmortality"))
					{
						...
						setType(GOD);
						System.out.println(name + " has been promoted to the status of God. " + name + " is now a God.");
					}
					else
					{
				...
				//		System.out.println(personOwner.getName() + " attacks " + name);
						suffer(1);
				//		System.out.println(name + " loses 1 health. " + name + " now has " + health + " health.");
						thing.changeOwner(this);
					}
				}
			}
	...
	public void give(Thing thing, Person gPerson)
	{
		...
		if ((thing.getName()).equals("LightningBolt") && (gPerson.getName()).equals("Hades"))
		{
			...
			gPerson.say("Thank you Hercules. As a token of my gratitude, I will send you to Mount Olympus to meet your father, Zeus.");
			System.out.println("Hercules finds himself magically transported to Mount Olympus.");
			System.out.println("At MountOlympus " + name + " says - " + "Father! I have finally proved my valor, and in exchange, you brother Hades has sent me here.");
	...
	public void suffer(int hits)
	{
		...
		if (health <= 0)
		{
			...
			{
				System.out.println("If you died when taking the lightning bolt from the LerneanHydra, here's a hint: You need to eat someone before taking the bolt otherwise the Hydra will drain the health that you were created with. If you eat someone, you have extra health.");
			}
			if (things.size() > 0)
			{
				for (int i = 0; i < things.size(); i++)
	...
	public void heroSuicide()
	{
		if (getThing("Elixir") != null) // this is a cheat that allows the user to bypass the game and win immediately
		{
			...
			System.out.println("Wait...if " + name + " is immortal, why is " + name + " not in olympus? Also, " + name + " should be a god since only gods are immortal.");
			System.out.println("Let's fix that.");
			System.out.println(name + " is now in Mount Olympus, and is a God, hence the objective of the game has been achieved.");
			setType(GOD);
		}
		else // a quick and easy way, if slightly gory, for the hero to die
		{
			say("I have decided to leave this world and never return.");
			System.out.println(name + " plunges his sword into his heart, convulses, and lies motionless on the floor.");
			if (things.size() > 0)
			{
				for (int i = 0; i < things.size(); i++)
				{
	...
	public void kill(Person victim)
	{
		if (getThing("Sword") != null)
		{
			...
			else if ((victim.getName()).equals("Minotaur"))
			{
				...
				System.out.println(victim.getName() + " charges and attempts to gore " + name + " on his golden horns.");
				System.out.println(name + " manages to slay " + victim.getName());
				room.remove(victim);
				victim.setRoom(null);
				suffer(2);
			...
			else // victim.getType() == GOD
			{
				...
				health -= 4;
				System.out.println(name + " loses 4 health for trying to kill a god." + name + " now has " + health + " health.");
			}
		}
		else
		{
			System.out.println(name + " has lost his sword! " + name + " must get it back before attempting to kill anyone.");
		}
	}
	...
	public void eatPerson(Person victim)
	{
		...
		if (victim.getType() != MONSTER && victim.getType() != GOD) // victim is a robot, gets eaten and gives health to hero
		{
			...
			System.out.println(name + " has increased health by " + victim.getHealth() + ". " + name + "'s health is now " + health);
		}
		else if (victim.getType() == MONSTER)
		{
			...
			System.out.println(victim.getName() + " fails to digest " + name + " and both " + victim.getName() + " and " + name + " die."); // person dies from trying to kill a monster
